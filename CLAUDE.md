...
- **AlphaClaw-owned credentials must never be written into the OpenClaw agent directory.** Anything only AlphaClaw reads back (today: the `x-twitter:*` cookie/OAuth1 profiles) goes to `<ALPHACLAW_DIR>/alphaclaw-profiles.json` (mode 0600) via the normal `auth-profiles.js` API — OpenClaw 2026.8.1+ treats `<OPENCLAW_DIR>/agents/<id>/agent/auth-profiles.json` as a *legacy* credential store and refuses the entire auth store while that file exists ("requires legacy credential migration; run openclaw doctor --fix"), which breaks every `openclaw` CLI call and every cron. Real model/provider credentials (Codex, Claude Code, `<provider>:default` API keys) still belong in the OpenClaw store. `auth-profiles.js` migrates any stray owned profile out of the legacy file on first access and deletes that file when nothing is left in it.
...
| `CRON_FAILURE_ALERT_ENABLED` | Set to `false` to disable cron failure alerting. Default `true`. |
| `CRON_FAILURE_ALERT_INTERVAL_MINUTES` | How often cron run history is polled for failures. Default `10`. |

**Codex debug-log pruning:** the Codex app-server writes an unbounded debug log to `~/.openclaw/agents/<agentId>/agent/codex-home/logs_2.sqlite` (~130 MB/day, no Codex-side retention setting), which fills the 5 GB volume in under two weeks. `lib/server/codex-log-prune.js` sweeps every agent's log DB ~2 minutes after boot and then every `CODEX_LOG_PRUNE_INTERVAL_HOURS`: it deletes rows older than `CODEX_LOG_RETENTION_DAYS` (default 3), truncates the WAL, and VACUUMs (followed by a second WAL truncate) only when the freelist holds more than 64 MB of reclaimable space. Sweeps never throw — a busy, full-volume or unreadable DB is logged and retried on the next tick — and the last run is reported at `GET /api/status` under `maintenance.codexLogPrune`.

The delete is **batched** (20 000 rows per statement, WAL truncated after each batch), because a single `DELETE FROM logs WHERE ts < ?` over a multi-GB database dirties nearly every page and, in WAL mode, writes all of them into `logs_2.sqlite-wal` on the same volume before committing. The first prod run failed exactly that way (`database or disk is full` on a 1.33 GB DB with 508 MB free). A sweep stops after 500 batches or 120 s (measured with `performance.now()`) and resumes on the next tick, yields to the event loop between batches (node:sqlite is synchronous, so the dashboard and watchdog would otherwise freeze for the whole sweep), and skips VACUUM when `fs.statfsSync` reports less free space than the rebuild needs (retained size + 128 MB margin).

**Stuck-WAL guard:** `PRAGMA wal_checkpoint(TRUNCATE)` does not throw when it cannot run — it returns `busy: 1` and leaves the WAL untouched, and any other connection holding a read transaction (the Codex app-server tailing its own log) is enough to pin it. The checkpoint result is therefore read via `.prepare(...).get()` and the `-wal` file is stat'd after every batch; if the checkpoint reports busy or the sidecar exceeds `kCodexLogPruneWalCeilingBytes` (64 MB), the sweep stops as `capped` with reason `wal-not-truncating`, skips VACUUM, and retries next tick. Without it a 160 MB prune under a held reader grew the WAL to 158 MB while reporting a clean run.

**Cron failure alerting:** the watchdog only monitors the gateway *process*, so a gateway that is up but answering every scheduled job with a provider error (as happened after the OpenClaw 2.0 upgrade, when every cron failed with a 400 "model not supported") reads as perfectly healthy and nothing reaches the operator. `lib/server/cron-failure-alert.js` closes that gap: it polls `cronService.getBulkJobRuns` ~3 minutes after boot and then every `CRON_FAILURE_ALERT_INTERVAL_MINUTES`, and pushes any new run with status `error` (or `completionStatus: "failed"`) through the **same watchdog notifier** — so cron failures land on whichever channel the watchdog already uses (Discord on prod, Telegram on local dev) and honour `WATCHDOG_NOTIFICATIONS_DISABLED`. They are sent with `eventType: "cron-failure"` rather than `"crash"`, deliberately: `"crash"` makes the Slack path capture the thread and reply later gateway *recovery* messages into it, which a cron failure has no business joining. Several jobs failing in one poll batch into a single message, one line per job. De-duplication is per job: the same error text alerts at most once an hour, but a *changed* error re-alerts immediately. A per-job watermark of the newest run already examined is persisted to `~/.alphaclaw/cron-failure-alert-state.json` so a restart never replays old failures (with no watermark it looks back 30 minutes at most). Polls never throw — a broken cron CLI is logged and retried next tick — and the last poll/alert is reported at `GET /api/watchdog/status` under `status.cronFailureAlert`.
