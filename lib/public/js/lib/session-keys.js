...
export const isDestinationSessionKey = (sessionKey = "") => {
  ...
  return (
    ...
    normalizedSessionKey.includes(":channel:")
  );
};
...
export const getSessionKind = (sessionKey = "") => {
  ...
  if (/:channel:([^:]+)$/.test(normalizedSessionKey)) return "channel";
  return "other";
};
...
export const getSessionDisplayLabel = (sessionRow = null) => {
  ...
  if (kind === "channel") {
    ...
    const channelTarget = String(channelMatch?.[1] || "").trim();
    return channelTarget ? `Discord Channel ${channelTarget}` : "Discord Channel";
  }
