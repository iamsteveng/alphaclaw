...
export const startClaudeCodeAuth = () =>
  fetch('/api/claude/auth/start', { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then((r) => r.json());

export const exchangeClaudeCodeAuth = (code) =>
  fetch('/api/claude/auth/exchange', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ code }),
  }).then((r) => r.json());

export const disconnectClaudeCode = () =>
  fetch('/api/claude/auth/disconnect', { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then((r) => r.json());

export const getClaudeCodeStatus = () =>
  fetch('/api/claude/auth/status').then((r) => r.json());

export const submitClaudeSetupToken = (token) =>
  fetch('/api/claude/auth/setup-token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export const fetchXListIngestStatus = async () => {
  const res = await authFetch("/api/x-list-ingest/status");
  return parseJsonOrThrow(res, "Could not load X list ingest status");
};

export const ensureXListIngest = async ({ listId = "", deliveryChannel = "", deliveryTo = "" } = {}) => {
  const res = await authFetch("/api/x-list-ingest/ensure", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ listId, deliveryChannel, deliveryTo }),
  });
  return parseJsonOrThrow(res, "Could not register X list ingest cron");
};

export const removeXListIngest = async () => {
  const res = await authFetch("/api/x-list-ingest", { method: "DELETE" });
  return parseJsonOrThrow(res, "Could not remove X list ingest cron");
};

export const fetchTradingCronsStatus = async () => {
  const res = await authFetch("/api/trading-crons/status");
  return parseJsonOrThrow(res, "Could not load trading crons status");
};

export const ensureTradingCrons = async ({ deliveryChannel = "", deliveryTo = "" } = {}) => {
  const res = await authFetch("/api/trading-crons/ensure", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ deliveryChannel, deliveryTo }),
  });
  return parseJsonOrThrow(res, "Could not register trading crons");
};

export const removeTradingCrons = async () => {
  const res = await authFetch("/api/trading-crons", { method: "DELETE" });
  return parseJsonOrThrow(res, "Could not remove trading crons");
};

export const fetchMarketRiskScoreStatus = async () => {
  const res = await authFetch("/api/market-risk-score/status");
  return parseJsonOrThrow(res, "Could not load market risk score status");
};

export const ensureMarketRiskScore = async ({ deliveryChannel = "", deliveryTo = "" } = {}) => {
  const res = await authFetch("/api/market-risk-score/ensure", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ deliveryChannel, deliveryTo }),
  });
  return parseJsonOrThrow(res, "Could not register market risk score cron");
};

export const removeMarketRiskScore = async () => {
  const res = await authFetch("/api/market-risk-score", { method: "DELETE" });
  return parseJsonOrThrow(res, "Could not remove market risk score cron");
};
