...
export const kFeaturedModelDefs = [
  ...
  // openai-codex/ is the picker-level key for "OpenAI via the Codex
  // subscription OAuth profile"; the plain openai/ key is the API-key path.
  // Codex first so a subscription-only user gets a chip that passes the Save
  // gate, with the API-key key as the fallback.
  {
    label: "GPT-5.5",
    preferredKeys: ["openai-codex/gpt-5.5", "openai/gpt-5.5"],
  },
  {
    label: "GPT-5.6 Sol",
    preferredKeys: ["openai-codex/gpt-5.6-sol", "openai/gpt-5.6-sol"],
  },
  {
