...
import { fetchTradingCronsStatus, ensureTradingCrons, removeTradingCrons } from "../../lib/api.js";
...
const kJobLabels = {
  "trading-watchlist-builder": { label: "Watchlist builder", schedule: "11:30 AM UTC · weekdays" },
  "trading-price-report":      { label: "Price report",      schedule: "Every 15 min, 1–3 PM UTC · weekdays" },
  "trading-eod-loop":          { label: "EOD learning loop", schedule: "8:10 AM UTC · weekdays" },
};
    ...
    <div class="bg-surface border border-border rounded-xl overflow-hidden">
      <div class="flex items-center gap-2.5 px-4 py-3 border-b border-border">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-fg-muted">Watchlist builder · Price report · EOD loop</p>
        </div>
        ${isRegistered
          ...
          ? html`
              <div class="space-y-2">
                ...
                <div class="flex items-center gap-1.5 text-xs pt-0.5">
                  <span class=${`w-1.5 h-1.5 rounded-full shrink-0 ${status?.finnhubConfigured ? "bg-status-success" : "bg-status-warning"}`}></span>
                  <span class="text-fg-muted">
                    Finnhub price feed: ${status?.finnhubConfigured
                      ? "configured"
                      : html`<span class="text-status-warning">FINNHUB_API_KEY not set — add via Settings → Environment</span>`}
                  </span>
                </div>
                ${deliveryDropdown}
          ...
          : html`
              <div class="space-y-2.5">
                <p class="text-xs text-fg-muted">
                  Registers 3 cron jobs: a daily watchlist builder (11:30 AM UTC), intraday price reports (every 15 min, 1–3 PM UTC), and an EOD learning loop (8:10 AM UTC). Weekdays only.
                </p>
