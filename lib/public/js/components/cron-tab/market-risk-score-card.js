    ...
    <div class="bg-surface border border-border rounded-xl overflow-hidden">
      <div class="flex items-center gap-2.5 px-4 py-3 border-b border-border">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs text-fg-muted">Daily 8:15 AM ET · Main agent</p>
        </div>
        ${isRegistered
          ...
          ? html`
              <div class="space-y-2">
                <div class="text-xs text-fg-muted space-y-0.5">
                  <div>Schedule: <span class="font-mono text-body">${status.job?.cron || "15 8 * * *"}</span> America/New_York</div>
          ...
          : html`
              <div class="space-y-2.5">
                <p class="text-xs text-fg-muted">
                  Registers a daily cron job that runs at 8:15 AM ET (1h15m before US market open) and runs the DXY/HYG/SPY/VIX risk scoring model.
                </p>
                ${deliveryDropdown}
                <${ActionButton}
                  idleLabel="Register"
