...
export const XListIngestCard = () => {
  ...
  const loadStatus = useCallback(async () => {
    try {
      const s = await apiFetch("/api/x-list-ingest/status");
      setStatus(s);
    } catch {
  ...
  const handleRegister = async () => {
    setRegistering(true);
    try {
      const r = await apiFetch("/api/x-list-ingest/ensure", { method: "POST" });
  ...
  const handleRemove = async () => {
    setRemoving(true);
    try {
      const r = await apiFetch("/api/x-list-ingest", { method: "DELETE" });
  ...
  const handleUpdate = async () => {
    setUpdating(true);
    try {
      await apiFetch("/api/x-list-ingest", { method: "DELETE" });
      const r = await apiFetch("/api/x-list-ingest/ensure", { method: "POST" });
