...
export const useWelcome = ({ onComplete }) => {
  ...
  useEffect(() => {
    fetch("/api/status")
      .then((r) => r.json())
      .then((d) => { if (d?.githubConfigured) setGithubConfigured(true); })
      .catch(() => {});
  }, []);

  const activeGroups = githubConfigured
    ? kWelcomeGroups.filter((g) => g.id !== "github")
    : kWelcomeGroups;

  const kSetupStepIndex = activeGroups.length;
  ...
  useEffect(() => {
    if (githubConfigured && step === -1) setStep(0);
  }, [githubConfigured, step, setStep]);
  ...
  const activeGroup = step >= 0 && step < kSetupStepIndex ? activeGroups[step] : null;
  ...
  const handleSubmit = async () => {
    ...
    if (invalidGroup) {
      ...
      setSetupError(null);
      setStep(Math.max(0, activeGroups.findIndex((group) => group.id === invalidGroup.id)));
      return;
    }
    if (loading) return;
    ...
    if (preflightError) {
      ...
      setSetupError(null);
      setStep(Math.max(0, activeGroups.findIndex((group) => group.id === "github")));
      return;
    }
  ...
  const goBack = () => {
    ...
    setFormError(null);
    setStep((prev) => Math.max(githubConfigured ? 0 : -1, prev - 1));
  };
  ...
  const goNext = async () => {
    ...
    setStep((prev) => Math.min(activeGroups.length - 1, prev + 1));
  };
