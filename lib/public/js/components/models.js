...
// openai-codex/ is the UI picker prefix; the gateway normalises to openai/ on write.
// Use this when comparing a picker key against a status/gateway key.
const normalizeModelKeyForCompare = (key) => {
  if (typeof key !== "string") return key;
  return key.startsWith("openai-codex/") ? `openai/${key.slice("openai-codex/".length)}` : key;
};

// Resolve a gateway model key (openai/gpt-X) to the catalog key (openai-codex/gpt-X)
// so the dropdown shows the correct selected option.
const resolveToCatalogKey = (modelKey, catalogModels) => {
  if (!modelKey) return modelKey;
  if (catalogModels.some((m) => m.key === modelKey)) return modelKey;
  const codexKey = modelKey.startsWith("openai/")
    ? `openai-codex/${modelKey.slice("openai/".length)}`
    : null;
  if (codexKey && catalogModels.some((m) => m.key === codexKey)) return codexKey;
  return modelKey;
};
...
export const Models = () => {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const currentModel = resolveToCatalogKey(modelStatus.modelKey || "", catalogModels);
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      if (modelDirty && targetModel) {
        ...
        if (activeModel && normalizeModelKeyForCompare(activeModel) !== normalizeModelKeyForCompare(targetModel)) {
