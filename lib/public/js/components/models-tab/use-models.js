...
export const useModels = (agentId) => {
  ...
  const [claudeCodeStatus, setClaudeCodeStatus] = useState(
    () => (useCache && kModelsTabCache?.claudeCodeStatus) || null,
  );
  ...
  const claudeCodeFetchState = useCachedFetch("/api/claude/auth/status", getClaudeCodeStatus, {
    maxAgeMs: 15000,
  });
  ...
  const refresh = useCallback(async () => {
    ...
    setError("");
    try {
      const [catalogResult, configResult, codex, claudeCode] = await Promise.all([
        catalogFetchState.refresh({ force: true }),
        configFetchState.refresh({ force: true }),
        codexFetchState.refresh({ force: true }),
        claudeCodeFetchState.refresh({ force: true }),
      ]);
      ...
      setClaudeCodeStatus(claudeCode || null);
      setProfileEdits({});
      setOrderEdits({});
      ...
      updateCache({
        ...
        claudeCodeStatus: claudeCode || null,
      });
    } catch (err) {
  ...
  const refreshClaudeCodeStatus = useCallback(async () => {
    try {
      ...
      setClaudeCodeStatus(claudeCode || null);
      updateCache({ claudeCodeStatus: claudeCode || null });
    } catch {
      setClaudeCodeStatus(null);
      updateCache({ claudeCodeStatus: null });
    }
  }, [updateCache]);
