...
import { exchangeCodexOAuth, disconnectCodex, startClaudeCodeAuth, exchangeClaudeCodeAuth, disconnectClaudeCode, submitClaudeSetupToken } from "../../lib/api.js";
import {
...
// Field data (labels, urls, placeholders, hints) derives from the Provider
// Registry via kProviderAuthFields. Only what the registry can't know stays
// here: mode structure (tab labels, profile suffixes, field kinds) and the
// OAuth flow definitions.
const kCodexOauthMode = { id: "oauth", label: "Codex OAuth", isCodexOauth: true };
const kClaudeCodeOauthMode = {
  id: "oauth",
  label: "Claude.ai OAuth",
  isClaudeCodeOauth: true,
};

// Pseudo-providers: OAuth Auth Profiles (see CONTEXT.md), not Model Providers —
// they have no registry entry and their cards are fully bespoke.
const kPseudoProviderMeta = {
  "openai-codex": { label: "OpenAI Codex", modes: [kCodexOauthMode] },
  "claude-code": { label: "Claude Code", modes: [kClaudeCodeOauthMode] },
};

// Per-provider mode structure, keyed by the registry field's env-var key so a
// registry reorder can never misapply a structure. Providers without an entry
// get one api_key mode per field, labelled with the field's own label.
const kProviderModeStructures = {
  anthropic: {
  ...
  openai: { OPENAI_API_KEY: { label: "API Key" } },
  google: { GEMINI_API_KEY: { label: "API Key" } },
};
...
const buildProviderModes = (provider) => {
  const fields = kProviderAuthFields[provider] || [];
  if (fields.length === 0) return [kDefaultMode];
  const structures = kProviderModeStructures[provider] || {};
  return fields.map((fieldDef) => {
    const structure = structures[fieldDef.key] || {};
    return {
      id: structure.id || "api_key",
      label: structure.label || fieldDef.label || "API Key",
      profileSuffix: structure.profileSuffix || "default",
      placeholder: fieldDef.placeholder || "...",
      hint: fieldDef.hint,
      url: fieldDef.url,
      field: structure.field || "key",
    };
  });
};

export const getProviderMeta = (provider) => {
  if (kPseudoProviderMeta[provider]) return kPseudoProviderMeta[provider];
  const modes = buildProviderModes(provider);
  // The Codex OAuth flow rides on the openai card when the registry says the
  // provider supports the oauth Auth Method.
  if (provider === "openai" && providerHasOAuth(provider)) {
    modes.push(kCodexOauthMode);
  }
  return {
...
const ClaudeCodeOAuthSection = ({ claudeCodeStatus, onRefreshClaudeCode }) => {
  ...
  const handleSetupToken = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await submitClaudeSetupToken(token);
      if (!result.ok) throw new Error(result.error || 'Token rejected');
  ...
  if (claudeCodeStatus?.configured) {
    return html`
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          <span class="text-xs text-fg-muted">Claude.ai subscription</span>
        ...
        <div class="flex gap-2 flex-wrap">
          <button onclick=${handleCopyToken} class="text-xs font-medium px-3 py-1.5 rounded-lg ac-btn-secondary">
            Copy token
          </button>
          <button onclick=${handleDisconnect} class="text-xs font-medium px-3 py-1.5 rounded-lg ac-btn-ghost">
            Disconnect
          </button>
        </div>
      </div>
  ...
  return html`
    <div class="space-y-2">
      ...
      <p class="text-xs text-fg-muted">
        Run <span class="font-mono bg-field px-1 rounded">claude setup-token</span> in your terminal, then paste the token below.
      </p>
      <input
        type="password"
        value=${tokenInput}
        ...
        class="w-full bg-field border border-border rounded-lg px-3 py-2 text-xs text-body outline-none focus:border-fg-muted font-mono"
      />
      <${ActionButton}
      ...
      <div class="pt-1">
        <button
          ...
          class="text-xs text-fg-dim hover:text-fg-muted"
        >
          ${showOAuth ? '▾' : '▸'} Advanced: OAuth flow
        </button>
        ${showOAuth ? html`
          <div class="space-y-2 mt-2 pl-2 border-l border-border">
            ${authStarted ? html`
              <p class="text-xs text-fg-muted">
                After authorizing, your browser redirects to localhost — copy the full URL from the address bar and paste below.
              </p>
              ...
              <input
                ...
                placeholder="Paste redirect URL (http://localhost:44889/callback?code=...)"
                class="w-full bg-field border border-border rounded-lg px-3 py-2 text-xs text-body outline-none focus:border-fg-muted"
              />
              <div class="flex gap-2">
                <${ActionButton}
            ...
            ` : html`
              <button onclick=${handleStart} class="text-xs font-medium px-3 py-1.5 rounded-lg ac-btn-secondary">
                Start OAuth flow
              </button>
            `}
          </div>
...
export const ProviderAuthCard = ({
  ...
  const isConnected =
    ...
    }) || ((provider === "openai-codex" || provider === "openai") && !!codexStatus?.connected)
