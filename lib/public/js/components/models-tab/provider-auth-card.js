    ...
    <div class="space-y-2">
      ...
      ${claudeCodeStatus?.configured && !authStarted
        ...
        ? html`
            <div class="space-y-2">
              <p class="text-xs text-fg-muted">
                Authorize on Claude.ai. Your browser will redirect to localhost and show an error — that's expected. Copy the full URL from your address bar and paste it below.
              </p>
              <a
                href=${authUrl}
                target="_blank"
              ...
              <input
                ...
                placeholder="Paste redirect URL (http://127.0.0.1:54321/?code=...)"
                class="w-full bg-field border border-border rounded-lg px-3 py-2 text-xs text-body outline-none focus:border-fg-muted"
              />
              <${ActionButton}
