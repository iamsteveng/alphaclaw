...
import { XAuthCard } from "../x-tab/x-auth-card.js";
import { XWebSessionCard } from "../x-tab/x-web-session-card.js";
import {
  getFeaturedModels,
  kProviderOrder,
  providerHasOAuth,
...
const deriveRequiredProviders = (configuredModels) => {
  ...
  for (const modelKey of Object.keys(configuredModels)) {
    ...
    // openai-codex/ models are shown in the unified OpenAI card (API key + Codex OAuth)
    if (provider === "openai-codex") providers.add("openai");
    else if (provider) providers.add(provider);
  }
  return [...providers];
};
...
const kProviderDisplayOrder = [
  "anthropic",
  "openai",
  // The Claude Code pseudo-card exists only while the registry says anthropic
  // supports the oauth Auth Method.
  ...(providerHasOAuth("anthropic") ? ["claude-code"] : []),
  ...kProviderOrder.filter((provider) => !["anthropic", "openai", "openai-codex"].includes(provider)),
];
...
export const Models = ({ onRestartRequired = () => {}, agentId, embedded = false }) => {
  ...
  const kAlwaysShownProviders = new Set(["claude-code"]);

  const sortedProviders = useMemo(() => {
    const ordered = [];
    for (const p of kProviderDisplayOrder) {
      if (requiredProviders.includes(p) || kAlwaysShownProviders.has(p)) ordered.push(p);
    }
  ...
  const configuredModelEntries = useMemo(
    () =>
      Object.keys(configuredModels).map((key) => {
        ...
        const canBePrimary = !catalogEntry?.modality || catalogEntry.modality === "text";
        return {
          key,
            ...
            ${sortedProviders.map(
              (provider) => html`
                <${ProviderAuthCard}
                  ...
                  onRefreshClaudeCode=${refreshClaudeCodeStatus}
                />
              `,
            )}
          </div>
