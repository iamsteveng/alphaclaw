...
export const useModelCard = ({
  ...
  const authorizedModelOptions = useMemo(
    () =>
      Object.keys(configuredModels || {})
        ...
        .filter((model) => {
          ...
          if (providerHasAuth[provider]) return true;
          // Older saves normalised openai-codex/ → openai/ in configuredModels.
          // Treat openai/ models as authorised when the catalog has a matching
          // openai-codex/ entry and Codex OAuth is connected.
          if (
            codexStatus?.connected &&
            getModelCatalogProvider(model) === "openai" &&
            catalog.some(
              (m) =>
                m.key ===
                `openai-codex/${String(model.key).slice("openai/".length)}`,
            )
          )
            return true;
          return false;
    ...
    [catalog, configuredModels, providerHasAuth, codexStatus],
  );
