...
Save the full analysis as a brain page (use lowercase ticker in slug):
- **Slug:** `stocks/variant-perception-[ticker]-[YYYY-MM-DD]`
- First run: `gbrain restore stocks/variant-perception-[ticker]-[YYYY-MM-DD] 2>/dev/null || true`
- Then: `gbrain put stocks/variant-perception-[ticker]-[YYYY-MM-DD]` with the full analysis
- **Type:** analysis (or concept)
- **Tags:** `variant-perception`, `stocks`, ticker symbol (lowercase)
...
For each section, replace each question with its answer — do not print the question text. Do not skip any section.
