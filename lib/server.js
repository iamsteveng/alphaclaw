...
const { createOmcVersionService } = require("./server/omc-version");
...
} = require("./server/acp-agent-config");
const {
  ensureGatewayProviderConfig,
} = require("./server/gateway-provider-config");
const {
...
const omcVersionService = createOmcVersionService({ gatewayEnv });
...
startServerLifecycle({
  ...
  ensureGatewayProviderConfig: () => {
    // An env var "has a value" if it is set in process.env OR present with a
    // non-empty value in the persisted env file (the source the old glm boot
    // check consulted).
    const envRows = readEnvFile() || [];
    const hasEnvValue = (envVar) =>
      String(process.env[envVar] || "").trim().length > 0 ||
      envRows.some(
        (entry) => entry?.key === envVar && String(entry?.value || "").trim(),
      );
    return ensureGatewayProviderConfig({
      fsModule: fs,
      openclawDir: constants.OPENCLAW_DIR,
      hasEnvValue,
    });
