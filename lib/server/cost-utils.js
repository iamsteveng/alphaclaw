...
const kGlobalModelPricing = {
  ...
  // `resolvePricingFromFallbackMap` falls back to `normalized.includes(key)` over
  // these keys in insertion order, so the most specific model id must come first
  // (otherwise "openai/gpt-5.5" would match the "gpt-5" row).
  // OpenAI pricing checked 2026-09-05 against https://developers.openai.com/api/docs/pricing
  ...
  // The pricing page lists no bare `gpt-5.6` entry; it is priced as Sol here on
  // the strength of OpenClaw's docs calling `openai/gpt-5.6` a Sol alias.
  "gpt-5.6": { input: 4.0, output: 20.0, cacheRead: 0.4 },
