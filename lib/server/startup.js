...
const { startCronFailureAlert } = require('./cron-failure-alert');
...
const runOnboardedBootSequence = ({
  ...
  // Cron failure alerting (see lib/server/cron-failure-alert.js). Injectable so
  // tests can assert the boot wiring without arming real timers.
  startCronFailureAlertScheduler = startCronFailureAlert,
  ...
  try {
    // The watchdog only watches the gateway process; this watches the work the
    // gateway is supposed to be doing. Unref'd timers only — it must never keep
    // the process alive or throw into the boot sequence.
    ...
    console.error(
      `[alphaclaw] Failed to schedule cron failure alerting (non-fatal): ${error.message}`,
    );
  }
