...
// The gateway rejects a connect unless its PROTOCOL_VERSION falls within the
// [min, max] the client offers (it checks `maxProtocol >= N && minProtocol <= N`).
// Offer a range instead of a single pinned version so the bridge survives a
// gateway protocol bump without a "protocol mismatch" outage. OpenClaw moved
// from 3 -> 4 (2026.6.x) while keeping the connect/req/res framing identical.
const kGatewayMinProtocolVersion = 3;
const kGatewayMaxProtocolVersion = 4;
...
const sanitizeError = (error) => {
  ...
  if (lower.includes("protocol mismatch") || lower.includes("protocol_mismatch")) {
    return "The gateway uses a newer protocol than this AlphaClaw build. Update AlphaClaw's gateway bridge protocol range.";
  }
    ...
    if (!gatewayConnectPromise) {
      gatewayConnectPromise = withTimeout(
        new Promise((resolve, reject) => {
          ...
          const connectParams = {
            minProtocol: kGatewayMinProtocolVersion,
            maxProtocol: kGatewayMaxProtocolVersion,
            client: {
              id: "gateway-client",
              version: "0.1.0",
