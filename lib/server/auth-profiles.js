...
const { GLM_PROVIDER_ID, ensureGlmProviderEntry } = require("./glm-provider-config");
...
const hasGlmApiKeyValue = (credential) =>
  credential?.type === "api_key" &&
  String(credential?.key || "").trim().length > 0;

// Delegates to the shared ensureGlmProviderEntry (glm-provider-config.js) so
// this stays consistent with the boot-time registration: the models.providers.glm
// block (with its ${GLM_API_KEY} secret ref) must only exist when a real key
// value is present, since OpenClaw's gateway hard-fails startup on any
// unresolvable secret ref — an empty/cleared key must remove the block, not
// just leave it in place pointing at nothing.
const syncGlmProviderConfig = (cfg, { hasApiKey }) => {
  const next = { ...cfg, models: { ...cfg.models, providers: { ...cfg.models?.providers } } };
  ensureGlmProviderEntry(next, { hasApiKey });
  return next;
};
...
const createAuthProfiles = () => {
  ...
  const upsertProfile = (profileId, credential, agentId = kDefaultAgentId) => {
    ...
    if (sanitized.provider === GLM_PROVIDER_ID) {
      updated = syncGlmProviderConfig(updated, { hasApiKey: hasGlmApiKeyValue(sanitized) });
    }
  ...
  const syncConfigAuthReferencesForAgent = (agentId = kDefaultAgentId) => {
    ...
    let hasGlmApiKey = false;
    for (const [profileId, credential] of Object.entries(store.profiles || {})) {
      if (!credential?.type || !credential?.provider) continue;
      cfg = syncConfigAuthReference(cfg, profileId, credential);
      if (credential.provider === GLM_PROVIDER_ID && hasGlmApiKeyValue(credential)) {
        hasGlmApiKey = true;
      }
    }
    cfg = syncGlmProviderConfig(cfg, { hasApiKey: hasGlmApiKey });
