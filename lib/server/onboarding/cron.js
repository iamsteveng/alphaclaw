...
const installHourlyGitSyncCron = async ({ fs, openclawDir }) => {
  try {
    ...
    // On re-installs (boot cycles), read existing config rather than overwriting user settings.
    let config;
    if (fs.existsSync(configPath)) {
      try { config = JSON.parse(fs.readFileSync(configPath, "utf8")); } catch { /* use defaults */ }
    }
    if (!config) {
      config = { enabled: true, schedule: kDefaultSystemCronSchedule };
      fs.mkdirSync(configDir, { recursive: true });
      fs.writeFileSync(configPath, JSON.stringify(config, null, 2));
    }

    if (!config.enabled) {
      console.log("[onboard] Hourly git sync cron disabled, skipping system cron install");
      return false;
    }

    const cronContent = buildSystemCronFile({
      schedule: config.schedule || kDefaultSystemCronSchedule,
