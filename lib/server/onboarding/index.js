...
const { promoteCloneToTarget } = require("./import/import-applier");
const {
  buildOnboardArgs,
...
// System vars (e.g. OPENCLAW_GATEWAY_TOKEN, WEBHOOK_TOKEN) are per-instance
// secrets, excluded from the user-facing placeholder review, and never
// carried over by an import — an imported config only ever references them
// via "${VAR}". Generate a fresh value for any that are referenced but not
// already present, so the gateway doesn't start with an unresolved secret ref.
const ensureImportedSystemVarSecrets = ({
  fs,
  openclawDir,
  varsToSave,
  systemVars = new Set(),
  ...
  const referencedEnvVars = getImportedConfigEnvRefs({ fs, openclawDir });
  for (const envKey of referencedEnvVars) {
    if (!systemVars.has(envKey)) continue;
    if (String(getEnvVarValue(varsToSave, envKey) || "").trim()) continue;
    upsertEnvVar(varsToSave, envKey, crypto.randomBytes(32).toString("hex"));
  }
};

const readOpenclawConfigSafe = ({ fs, openclawDir }) => {
  try {
    return JSON.parse(fs.readFileSync(`${openclawDir}/openclaw.json`, "utf8"));
  } catch {
    return null;
  }
};

// The `openclaw models set` CLI can rewrite openclaw.json from its own
// narrower internal view, silently dropping top-level sections (gateway,
// channels, plugins, ...) it doesn't manage — fatal for an imported config,
// since a missing gateway.mode blocks the gateway from starting at all.
// Restore any section that existed before the CLI ran but disappeared
// afterward, without touching whatever it legitimately wrote for agents/auth.
const restoreDroppedConfigSections = ({ fs, openclawDir, preConfig }) => {
  if (!preConfig || typeof preConfig !== "object") return;
  const postConfig = readOpenclawConfigSafe({ fs, openclawDir });
  if (!postConfig || typeof postConfig !== "object") return;
  let changed = false;
  for (const [key, value] of Object.entries(preConfig)) {
    if (key === "agents" || key === "auth") continue;
  ...
  if (changed) {
    ...
    console.log(
      "[onboard] Restored config sections dropped by openclaw models set",
    );
  }
};
...
const createOnboardingService = ({
  ...
  const completeOnboarding = async ({
    ...
    // Detect whether github vars came from env (not submitted in the vars array)
    const githubVarsFromEnv =
      Array.isArray(vars) &&
      !vars.find((v) => v.key === "GITHUB_TOKEN") &&
      !vars.find((v) => v.key === "GITHUB_WORKSPACE_REPO");
    ...
    if (githubVarsFromEnv && !importMode) {
      // Auto-detect import mode: non-empty remote repo → import; empty/absent → fresh setup
      const repoStatus = await verifyGithubRepoForOnboarding({
        repoUrl,
        githubToken,
        mode: "existing",
      });
      if (repoStatus.ok && repoStatus.repoExists && !repoStatus.repoIsEmpty) {
        resolvedImportMode = true;
      } else {
        const ensureResult = await ensureGithubRepoAccessible({
          repoUrl,
          repoName,
          githubToken,
        });
        if (!ensureResult.ok) {
          return {
            status: ensureResult.status || 400,
            body: { ok: false, error: ensureResult.error },
          };
        }
      }
    } else {
    ...
    // Headless auto-import: clone remote repo content into OPENCLAW_DIR
    if (githubVarsFromEnv && resolvedImportMode) {
      fs.mkdirSync(OPENCLAW_DIR, { recursive: true });
      const cloneResult = await cloneRepoToTemp({ repoUrl, githubToken, shellCmd });
      if (!cloneResult.ok) {
        return { status: 400, body: { ok: false, error: cloneResult.error } };
      }
      const promoteResult = promoteCloneToTarget({
        fs,
        tempDir: cloneResult.tempDir,
        targetDir: OPENCLAW_DIR,
        sourceSubdir: "",
        cleanupBootstrap: false,
      });
      cleanupTempClone(cloneResult.tempDir);
      if (!promoteResult.ok) {
        return {
          status: 500,
          body: { ok: false, error: promoteResult.error || "Failed to apply imported workspace" },
        };
      }
      console.log("[onboard] Headless import: cloned remote repo content into OPENCLAW_DIR");
    }

    const existingConfigPresent =
      resolvedImportMode && fs.existsSync(`${OPENCLAW_DIR}/openclaw.json`);

    if (resolvedImportMode && existingConfigPresent) {
      const systemVars =
    ...
    const hadImportedGit = resolvedImportMode && fs.existsSync(`${OPENCLAW_DIR}/.git`);
    if (hadImportedGit) {
      try {
    ...
    const preModelSetConfig = readOpenclawConfigSafe({
      fs,
      openclawDir: OPENCLAW_DIR,
    });
    ...
    try {
      const commitMsg = resolvedImportMode
