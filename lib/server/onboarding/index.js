...
const { listProviders } = require("../model-providers");
const {
...
const syncApiKeyAuthProfilesFromEnvVars = (authProfiles, envVars = []) => {
  ...
  // Derived from the Provider Registry (was a hardcoded copy that silently
  // omitted glm/deepseek — deriving here fixes that latent bug so those
  // providers' api-key profiles are synced from env vars like the rest).
  const providers = listProviders().map((entry) => entry.id);
...
const createOnboardingService = ({
  ...
  getBaseUrl,
  startGateway,
  // Which OpenClaw is installed decides whether exec approvals still live in
  // `exec-approvals.json` (pre-2026.8.1) or in SQLite. Injectable so the
  // behaviour under test does not depend on the version in node_modules.
  resolveOpenclawVersion = resolveInstalledOpenclawVersion,
}) => {
