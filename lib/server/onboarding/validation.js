...
const validateOnboardingInput = ({ vars, modelKey, resolveModelProvider, hasCodexOauthProfile, readEnvFile }) => {
  ...
  // Fall back to .env file values for GitHub vars when absent from submitted vars
  const envFileVars = typeof readEnvFile === "function" ? readEnvFile() : [];
  const githubToken = String(varMap.GITHUB_TOKEN || envFileVars.find((v) => v.key === "GITHUB_TOKEN")?.value || "");
  const githubRepoInput = String(varMap.GITHUB_WORKSPACE_REPO || envFileVars.find((v) => v.key === "GITHUB_WORKSPACE_REPO")?.value || "").trim();
