...
// Copies skills from lib/setup/skills/ into workspace/skills/ on every startup.
// OpenClaw's gateway scans workspace/skills/ via resolvePluginSkillDirs and makes
// them available to the agent as openclaw-workspace skills.
const syncWorkspaceSkills = ({ fs, workspaceDir }) => {
  try {
    const skillsSourceDir = path.join(kSetupDir, "skills");
    if (!fs.existsSync(skillsSourceDir)) return;
    for (const skill of fs.readdirSync(skillsSourceDir)) {
      const src = path.join(skillsSourceDir, skill);
      if (!fs.statSync(src).isDirectory()) continue;
      const dest = path.join(workspaceDir, "skills", skill);
      fs.mkdirSync(dest, { recursive: true });
      for (const file of fs.readdirSync(src)) {
        const fileSrc = path.join(src, file);
        if (!fs.statSync(fileSrc).isFile()) continue;
        fs.copyFileSync(fileSrc, path.join(dest, file));
      }
    }
  } catch (e) {
    console.error("[onboard] Workspace skills sync error:", e.message);
  }
};
