...
const normalizeCodexModelKey = (key) => {
  if (typeof key !== "string") return key;
  return key.startsWith("openai-codex/")
    ? `openai/${key.slice("openai-codex/".length)}`
    : key;
};

const normalizeCodexConfiguredModels = (configuredModels) => {
  if (!configuredModels || typeof configuredModels !== "object") return configuredModels;
  const result = {};
  for (const [key, value] of Object.entries(configuredModels)) {
    if (key.startsWith("openai-codex/")) {
      const normalizedKey = `openai/${key.slice("openai-codex/".length)}`;
      const entry = typeof value === "object" && value !== null ? { ...value } : {};
      if (!entry.agentRuntime) entry.agentRuntime = {};
      if (!entry.agentRuntime.id) entry.agentRuntime.id = "codex";
...
const normalizeOnboardingModels = (models) => {
  ...
  // The live OpenClaw catalog normalises everything to openai/ — inject
  // openai-codex/ entries from the bootstrap catalog so they survive a
  // live refresh.
  for (const model of kBootstrapModelCatalog.models || []) {
    if (!model?.key || typeof model.key !== "string") continue;
    if (resolveModelProvider(model.key) !== "openai-codex") continue;
    if (!deduped.has(model.key)) {
      deduped.set(model.key, {
        key: model.key,
