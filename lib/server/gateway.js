...
const _readPluginPathsGw = () => {
  const readPaths = (filePath) => {
    try {
      const cfg = JSON.parse(fs.readFileSync(filePath, 'utf8'));
      return (cfg?.plugins?.load?.paths || []).join(', ') || '(empty)';
    } catch (e) { return `(err: ${e.message})`; }
  };
  const data = readPaths(path.join(OPENCLAW_DIR, 'openclaw.json'));
  const root = readPaths('/root/.openclaw/openclaw.json');
...
const prepareOpenclawChannelPlugins = () => {
  ...
  console.log(`[alphaclaw] preflight: plugin paths on disk = ${_readPluginPathsGw()}`);
  try {
    runPluginRuntimeDepsPreflight();
  } catch (err) {
    console.log(`[alphaclaw] preflight: plugin paths after failed preflight = ${_readPluginPathsGw()}`);
