...
const kGatewayRestartCmdTimeoutMs = 60 * 1000;
...
const kStaleVolumeCacheDirs = () => [
  path.join(kRootDir, ".npm", "_cacache"),
  resolveOpenclawCompileCacheDir(),
];

const cleanupStaleVolumeCaches = () => {
  for (const cacheDir of kStaleVolumeCacheDirs()) {
    try {
      ...
      console.log(`[alphaclaw] Cleared stale volume cache: ${cacheDir}`);
    } catch (err) {
      console.warn(
        `[alphaclaw] Could not clear volume cache ${cacheDir}: ${err.message}`,
      );
    }
  }
};
...
const runGatewayCmd = (cmd) => {
  ...
  try {
    ...
    const out = execSync(`openclaw gateway ${cmd}`, {
      env: gatewayEnv(),
      timeout: kGatewayRestartCmdTimeoutMs,
      encoding: "utf8",
    });
