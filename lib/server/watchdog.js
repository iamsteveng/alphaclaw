...
// Auto-repair defaults ON: a gateway that crashes and never comes back is
// worse than an unwanted repair attempt. Set WATCHDOG_AUTO_REPAIR=false to
// opt out on a specific deployment.
const resolveAutoRepairDefault = (value) => parseBooleanValue(value, true);
...
const createWatchdog = ({
  ...
  const state = {
    ...
    repairAttempts: 0,
    crashTimestamps: [],
    autoRepair: resolveAutoRepairDefault(process.env.WATCHDOG_AUTO_REPAIR),
  ...
  const updateSettings = ({ autoRepair, notificationsEnabled } = {}) => {
    ...
    reloadEnv();
    state.autoRepair = resolveAutoRepairDefault(process.env.WATCHDOG_AUTO_REPAIR);
  ...
  const runRepair = async ({ source, correlationId, force = false }) => {
    ...
    try {
      // doctor --fix can run several minutes; the 15s default would kill it
      // mid-repair and log the truncation as a repair failure.
