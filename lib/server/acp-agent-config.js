...
const ensureAcpEntry = (cfg = {}) => {
  ...
  if (!cfg.agents.list.some((agent) => agent?.id === "claude")) {
    cfg.agents.list.push({
      ...
      runtime: {
        type: "acp",
        // Without an explicit timeoutSeconds here, a stuck turn falls back to
        // the general agent command timeout (default 600s) instead of acpx's
        // own 120s default — a hang reads as total silence for up to 10
        // minutes instead of failing fast with a visible error.
        acp: {
          agent: "claude",
          backend: "acpx",
          mode: "persistent",
