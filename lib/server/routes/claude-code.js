...
const registerClaudeCodeRoutes = ({ app, authProfiles, readEnvFile, writeEnvFile }) => {
  ...
  app.post('/api/claude/auth/setup-token', async (req, res) => {
    ...
    try {
      ...
      if (!validationRes.ok) {
        const json = await validationRes.json().catch(() => ({}));
        return res.status(400).json({ ok: false, error: `Token rejected by Anthropic (HTTP ${validationRes.status}): ${json?.error?.message || ''}` });
      }
      writeCredentials({ accessToken: token, refreshToken: '', expiresAt: 0, scopes: ['user:inference'] });
      upsertEnvToken(token, readEnvFile, writeEnvFile);
      process.env.CLAUDE_CODE_OAUTH_TOKEN = token;
      if (authProfiles?.upsertClaudeCodeProfile) {
        authProfiles.upsertClaudeCodeProfile({ accessToken: token, refreshToken: '', expiresAt: 0, scopes: ['user:inference'] });
      }
      console.log('[claude-code] Setup token accepted and stored');
      return res.json({ ok: true });
    } catch (err) {
      console.error('[claude-code] Setup token validation error:', err.message);
      return res.status(500).json({ ok: false, error: err.message || 'Validation failed' });
    }
  });
