...
const { getCronFailureAlertStatus } = require("../cron-failure-alert");
...
const registerWatchdogRoutes = ({
  ...
  app.get("/api/watchdog/status", requireAuth, (req, res) => {
    try {
      // Cron failure alerting shares the watchdog's notification channel, so its
      // state belongs next to gateway health rather than in its own endpoint.
      const status = {
        ...watchdog.getStatus(),
        cronFailureAlert: getCronFailureAlertStatus(),
      };
