...
const { configureXurl } = require('../x-twitter-auth');
const {
  X_WEB_SESSION_PROFILE_ID,
  buildApiKey,
  verifyWebSession,
} = require('../x-web-session');
...
const registerXTwitterRoutes = ({ app, authProfiles }) => {
  ...
  app.post('/api/x/auth', async (req, res) => {
    ...
    try {
      ...
      configureXurl({ consumerKey: creds.consumerKey, consumerSecret: creds.consumerSecret, accessToken: creds.accessToken, accessSecret: creds.accessSecret });
  ...
  // ── Free crawl: cookie-based web session ────────────────────────────────────
  // Stores a logged-in browser session (auth_token + ct0 + twid [+ kdt]) used by
  // the x-list-crawl CLI. Not the official API — see the ADR.

  // POST /api/x/web-session
  // Body: { authToken, ct0, twid, kdt? }
  app.post('/api/x/web-session', async (req, res) => {
    const { authToken, ct0, twid, kdt } = req.body || {};
    let apiKey;
    try {
      apiKey = buildApiKey({
        authToken: String(authToken || '').trim(),
        ct0: String(ct0 || '').trim(),
        twid: String(twid || '').trim(),
        kdt: String(kdt || '').trim(),
      });
    } catch (err) {
      return res.status(400).json({ ok: false, error: err.message });
    ...
    try {
      const { userId, username, name } = await verifyWebSession({ apiKey });
      authProfiles.upsertProfile(X_WEB_SESSION_PROFILE_ID, {
        type: 'cookie',
        provider: 'x-twitter',
        access: apiKey,
        userId,
      ...
      console.log(`[x-twitter] Web session stored. User: @${username} (id=${userId})`);
      return res.json({ ok: true, username, userId, name });
    } catch (err) {
      console.error('[x-twitter] Web session validation failed:', err.message);
      return res.status(400).json({ ok: false, error: err.message });
    }
  });

  // GET /api/x/web-session/status
  app.get('/api/x/web-session/status', (req, res) => {
    const profile = authProfiles.getProfile(X_WEB_SESSION_PROFILE_ID);
    if (!profile?.access) return res.json({ configured: false });
    return res.json({
      configured: true,
      username: profile.username || null,
      userId: profile.userId || null,
  ...
  // POST /api/x/web-session/verify — live-test the stored cookies
  app.post('/api/x/web-session/verify', async (req, res) => {
    const profile = authProfiles.getProfile(X_WEB_SESSION_PROFILE_ID);
    if (!profile?.access) {
      return res.status(400).json({ ok: false, error: 'No X web session configured' });
    }
    try {
      const { userId, username, name } = await verifyWebSession({ apiKey: profile.access });
      return res.json({ ok: true, username, userId, name });
    } catch (err) {
      return res.status(400).json({ ok: false, error: err.message });
    }
  ...
  // DELETE /api/x/web-session
  app.delete('/api/x/web-session', (req, res) => {
    authProfiles.removeProfile(X_WEB_SESSION_PROFILE_ID);
    console.log('[x-twitter] Web session removed');
    return res.json({ ok: true });
  });
};
