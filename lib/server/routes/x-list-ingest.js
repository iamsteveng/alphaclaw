...
// X list IDs are numeric snowflakes. Returns the trimmed id, or null if invalid.
const parseListId = (raw) => {
  const s = String(raw ?? "").trim();
  return /^\d+$/.test(s) ? s : null;
};

// The list ID is stored only in the registered cron job's message. Recover it
// so the dashboard can display/prefill the current value. Best-effort: if the
// gateway doesn't echo the message back, returns null and the field starts empty.
const extractListIdFromJob = (job) => {
  const message = job?.payload?.message ?? job?.message ?? "";
  const match = String(message).match(/X list ID (\d+)/);
...
// profile. Without it every run fails auth, so registration is gated on it
// being present — alongside a valid list ID.
...
const registerXListIngestRoutes = ({ app, requireAuth, gatewayRpc, authProfiles }) => {
  app.get("/api/x-list-ingest/status", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const job = findCronJob(result?.jobs);
      res.json({ ok: true, listId: extractListIdFromJob(job), webSessionConfigured, job });
    } catch {
      res.json({ ok: true, listId: null, webSessionConfigured, job: null });
    }
  });
  ...
  app.post("/api/x-list-ingest/ensure", requireAuth, async (req, res) => {
    ...
    if (!listId) {
      return res.status(400).json({ ok: false, error: "A numeric X list ID is required" });
    }
...
module.exports = { registerXListIngestRoutes, isWebSessionConfigured, parseListId, extractListIdFromJob };
