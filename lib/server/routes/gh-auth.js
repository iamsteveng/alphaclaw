...
const storeGhToken = (token) =>
  new Promise((resolve, reject) => {
    // Strip GITHUB_TOKEN from env — gh refuses to store credentials when it's set
...
const registerGhAuthRoutes = ({ app }) => {
    ...
    try {
      ...
      if (data.access_token) {
        try {
          ...
          console.error('[gh-auth] token storage failed:', storeErr.message);
          return res.status(500).json({ ok: false, status: 'error', error: 'Token received but gh CLI storage failed. Is gh installed?' });
        }
        const user = getGhUser();
        console.log('[gh-auth] Device flow completed successfully', user ? `as ${user}` : '');
        return res.json({ ok: true, status: 'success', user });
      }
  ...
  app.post('/api/gh/auth/logout', (req, res) => {
    try {
      ...
      console.warn('[gh-auth] logout warning (gh may not have been authenticated):', err.message);
    }
