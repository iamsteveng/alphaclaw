...
const { resolveDefaultAgentId } = require("../webhooks");
const https = require("https");
const { getCodexLogPruneStatus } = require("../codex-log-prune");
...
const registerSystemRoutes = ({
  ...
  const kFallbackAgentId = "main";
  // Shell-safe agent ids only: the resolved id is interpolated into the
  // openclaw command line.
  const kAgentIdPattern = /^[A-Za-z0-9_-]+$/;
  const isValidAgentId = (value) =>
    kAgentIdPattern.test(String(value || "").trim());
  const parseAgentIdFromSessionKey = (key = "") =>
    String(String(key || "").match(/^agent:([^:]+):/)?.[1] || "").trim();
  // Gateway default agent from openclaw.json (agents.list, honouring the
  // `default` marker) — falls back to "main" when unreadable/unset.
  const getGatewayDefaultAgentId = () => {
    try {
      const config = readOpenclawConfig({
        fsModule: fs,
        openclawDir: OPENCLAW_DIR,
      ...
      return isValidAgentId(defaultAgentId) ? defaultAgentId : kFallbackAgentId;
    } catch {
      return kFallbackAgentId;
    }
  };
  ...
  const getSessionReplyTarget = (sessionKey = "") => {
    ...
    const discordDirectMatch = key.match(/:discord:direct:([^:]+)$/);
    if (discordDirectMatch) {
      return {
        replyChannel: "discord",
        replyTo: String(discordDirectMatch[1] || ""),
      };
    }
    const discordChannelMatch = key.match(/:discord:channel:([^:]+)$/);
    if (discordChannelMatch) {
      return {
        replyChannel: "discord",
        replyTo: String(discordChannelMatch[1] || ""),
      };
    }
    return {
      replyChannel: "",
  ...
  const listSendableAgentSessions = async () => {
    // Large session sets can push this past the 15s default.
    const result = await clawCmd("sessions --json --all-agents", {
      quiet: true,
      timeoutMs: 60000,
    });
    if (!result.ok) {
      throw new Error(result.failureMessage || "Could not load agent sessions");
    }
  ...
  const buildStatusPayload = async () => {
    ...
    const alphaclawVersion =
      typeof alphaclawVersionService?.readAlphaclawVersion === "function"
        ...
        : null;
    const omcVersion =
      typeof omcVersionService?.readOmcVersion === "function"
        ? omcVersionService.readOmcVersion()
        : null;
    const fileVars = typeof readEnvFile === "function" ? readEnvFile() : [];
    const githubToken = String(fileVars.find((v) => v.key === "GITHUB_TOKEN")?.value || "").trim();
    const githubRepo = String(fileVars.find((v) => v.key === "GITHUB_WORKSPACE_REPO")?.value || "").trim();
    return {
      gateway: running
        ? "running"
        : configExists
      ...
      githubConfigured: !!(githubToken && githubRepo),
      maintenance: { codexLogPrune: getCodexLogPruneStatus() },
    };
  };
  ...
  app.post("/api/agent/message", async (req, res) => {
    ...
    // Agent id resolution: explicit request field, then the session key's
    // `agent:<id>:...` segment, then the gateway's configured default agent,
    // then "main".
    ...
    if (!requestedAgentId && sessionAgentId && !isValidAgentId(sessionAgentId)) {
      return res.status(400).json({ ok: false, error: "Invalid agent id" });
    }
    const agentId =
      requestedAgentId ||
      sessionAgentId ||
      getGatewayDefaultAgentId() ||
      kFallbackAgentId;
    // agentId is validated against kAgentIdPattern above, so it is shell-safe
    // without quoting.
    let command = `agent --agent ${agentId} --message ${shellEscapeArg(message)}`;
    if (sessionKey) {
    ...
    // Agent turns routinely take 30–120s; clawCmd's default 15s timeout kills
    // them mid-run.
    const result = await clawCmd(command, { quiet: true, timeoutMs: 300000 });
    if (!result.ok) {
      return res
        .status(502)
        .json({
          ok: false,
          error: result.failureMessage || "Could not send message to agent",
        });
    }
