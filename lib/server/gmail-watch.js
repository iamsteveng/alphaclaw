...
const createGmailWatchService = ({
  ...
  const runGogForAccount = async ({ account, command, quiet = true }) => {
    ...
    // Watch start/stop are remote Google API calls; 15s is too tight.
    return await gogCmd(`${prefix}${command}`, { quiet, timeoutMs: 60000 });
  };
  ...
  const startWatch = async ({ accountId, req, destination = null }) => {
    ...
    if (!watchStart.ok) {
      throw new Error(watchStart.failureMessage || "Failed to start Gmail watch");
    }
  ...
  const stopWatch = async ({ accountId }) => {
    ...
    if (!watchStop.ok) {
      console.log(
        `[alphaclaw] Gmail watch stop warning (${account.email}): ${watchStop.failureMessage || "unknown"}`,
      );
    }
    const updated = setAccountGmailWatch({
      state,
