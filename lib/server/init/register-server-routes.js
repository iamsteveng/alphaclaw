...
const { registerXTwitterRoutes } = require("../routes/x-twitter");
...
const { registerXListIngestRoutes } = require("../routes/x-list-ingest");
const { registerMarketRiskScoreRoutes } = require("../routes/market-risk-score");
const { registerTradingCronRoutes } = require("../routes/trading-crons");
const { registerGbrainCronRoutes } = require("../routes/gbrain-crons");
const { createGatewayRpc } = require("../gateway-rpc");
const { readOpenclawConfig } = require("../openclaw-config");
const { registerNodeRoutes } = require("../routes/nodes");
const { registerGhAuthRoutes } = require("../routes/gh-auth");
const {
...
const registerServerRoutes = ({
  ...
  registerXTwitterRoutes({ app, authProfiles });
  ...
  const getGatewayToken = () => {
    ...
    const cfg = readOpenclawConfig({ openclawDir: constants.OPENCLAW_DIR, fallback: {} });
    const raw = String(cfg?.gateway?.auth?.token || "").trim();
    if (!raw) return "";
    const m = raw.match(/^\$\{([A-Z0-9_]+)\}$/i);
    return m ? String(process.env[m[1]] || "").trim() : raw;
  };
  const gatewayRpc = createGatewayRpc({ getGatewayToken });
  registerCronRoutes({ app, requireAuth, cronService, gatewayRpc });
  registerXListIngestRoutes({ app, requireAuth, gatewayRpc, authProfiles });
  registerMarketRiskScoreRoutes({ app, requireAuth, gatewayRpc });
  registerTradingCronRoutes({ app, requireAuth, gatewayRpc });
  registerGbrainCronRoutes({ app, requireAuth, gatewayRpc });
