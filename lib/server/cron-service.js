...
const createCronService = ({
  ...
  const runCommand = async (command, { timeoutMs = 30000 } = {}) => {
    ...
    if (!result?.ok) {
      // failureMessage already distinguishes timeout/signal/exit; some CLIs
      // print errors to stdout, so keep that as the secondary fallback.
      const message = String(
        result?.failureMessage || result?.stdout || "Command failed",
      ).trim();
